...
def parse_args(args_list: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        '--number-step', type=int, default=1, help="Increment step for numbering (default: 1)"
    )
    parser.add_argument(
        '--number-padding', type=int, default=3, help="Zero padding width for numbers (default: 3)"
    )
    parser.add_argument(
        '--number-format', default='{name}_{num}', help="Format for numbering (default: {name}_{num})"
    )
    parser.add_argument(
        '--sort', choices=['name', 'mtime', 'size'], default='name',
        help="Sorting method before numbering (default: name)"
    )

    parser.add_argument(
        '--clean-dates', action='store_true', help="Clean and normalize dates in filenames to YYYY-MM-DD"
    )
    parser.add_argument(
        '--date-format', choices=['DMY', 'MDY', 'YMD'], default='YMD',
    ...
    parser.add_argument(
        '--replace-spaces', nargs='?', const='_', default=None,
        help="Replace spaces with optional char (default: '_')"
    )

    # Operational
    parser.add_argument(
        '--dry-run', action='store_true', help="Preview proposed changes without execution"
    )
    parser.add_argument(
        '--force', action='store_true', help="Skip user confirmation prompt"
    )
    parser.add_argument(
        '--history-file', default=None, help="Path to history file (default: .rename_history.json)"
    )
    parser.add_argument(
    ...
    if recursive:
        for root, _, files in os.walk(target_dir):
            for file in files:
                ...
                if full_path == history_file_abs or full_path == script_abs:
                    continue
...
def validate_renames(rename_list: List[Tuple[str, str]]) -> List[str]:
    ...
    """
    src_set_normalized = {os.path.abspath(os.path.normcase(src)) for src, _ in rename_list}
    ...
    for dest_norm, items in dest_normalized_to_srcs.items():
        if len(items) > 1:
            ...
            conflicts.append(
                f"Many-to-one collision: {len(items)} files want to rename to conflicting destinations: "
    ...
    for src, dest in rename_list:
        ...
        if os.path.exists(dest_abs) and dest_normalized not in src_set_normalized:
            conflicts.append(
                f"External collision: Target '{dest}' already exists on disk and is not in the source rename list (from '{src}')"
            )

    return conflicts


def print_preview_table(rename_list: List[Tuple[str, str]], target_dir: str, conflicts: List[str]) -> None:
    ...
    for conf in conflicts:
        # Simple parsing or match to identify which destination is in conflict
        # To be safe, we can check if a destination matches collision rules
        pass
...
def execute_2phase_rename(rename_list: List[Tuple[str, str]]) -> List[Tuple[str, str]]:
    ...
    try:
        ...
        sys.stderr.write(f"Phase 1 renaming failed midway: {e}. Reverting completed changes...\n")
        for src_path, tmp_path, _ in reversed(phase1_done):
            try:
                os.rename(tmp_path, src_path)
            except Exception as revert_err:
                sys.stderr.write(f"Failed to revert temp rename from '{tmp_path}' back to '{src_path}': {revert_err}\n")
        raise RuntimeError(f"Phase 1 failure: {e}")

    phase2_done = []
    try:
        for src, temp_path, dest in phase1_done:
            # Ensure target parent directory exists (needed in case renaming moves files, 
            # though our logic keeps them in same dir)
        ...
        sys.stderr.write(f"Phase 2 renaming failed midway: {e}. Reverting to original files...\n")
        for src_path, dest_path in reversed(phase2_done):
            # Find matching temp path
            for s, t, d in phase1_done:
                if s == src_path:
                    try:
                        os.rename(dest_path, t)
                    except Exception as revert_err:
                        sys.stderr.write(f"Failed to rename '{dest_path}' back to temp '{t}': {revert_err}\n")
                    break
        ...
        for src_path, tmp_path, _ in reversed(phase1_done):
            if os.path.exists(tmp_path):
                try:
                    ...
                    sys.stderr.write(f"Failed to revert temp '{tmp_path}' back to source '{src_path}': {revert_err}\n")
...
def execute_undo(history_file_path: str, target_dir: str) -> None:
    ...
    # 2. Verify all src files (original paths to restore) do not exist (unless part of the undo plan)
    ...
    if missing_files or existing_srcs:
        if missing_files:
            sys.stderr.write("Undo validation failed: The following files to be undone do not exist on disk:\n")
            for f in missing_files:
                sys.stderr.write(f"  - {f}\n")
        if existing_srcs:
            sys.stderr.write(
                "Undo validation failed: The following original paths already exist on disk "
                "(and are not part of the undo plan):\n"
            )
    ...
    try:
        ...
        for _, temp_path, dest in reversed(temp_renames):
            try:
                ...
                sys.stderr.write(f"Failed to revert temp rename from '{temp_path}' back to '{dest}': {revert_err}\n")
        sys.exit(1)
    ...
    for idx, (src, temp_path, dest) in enumerate(temp_renames):
        try:
            ...
            failed_index = idx
            sys.stderr.write(f"Failed to rename temp file '{temp_path}' to target '{src}': {e}\n")
            break

    if failed_index != -1:
        # Phase 2 failed midway!
        # Restore remaining temp files back to their current dest paths
        sys.stderr.write("Undo Phase 2 failed midway. Restoring remaining temp files...\n")
        for idx in range(failed_index, len(temp_renames)):
            _, temp_path, dest = temp_renames[idx]
            try:
                os.rename(temp_path, dest)
            except Exception as revert_err:
                sys.stderr.write(f"Critical: Failed to restore temp file '{temp_path}' back to '{dest}': {revert_err}\n")
        ...
        try:
            ...
            sys.stderr.write(f"Updated history file with remaining uncompleted renames.\n")
    ...
    try:
        ...
        sys.stderr.write(f"Warning: Failed to save/rename history log to .undone: {save_err}\n")
        sys.exit(0)
...
def main(args_list: Optional[List[str]] = None) -> None:
    ...
    if args.history_file is None:
        history_file_path = os.path.abspath(os.path.join(target_dir, ".rename_history.json"))
    else:
    ...
    # Sort files
    if args.sort == 'mtime':
        matched_files.sort(key=lambda p: (os.path.getmtime(p), os.path.relpath(p, target_dir)))
    elif args.sort == 'size':
        matched_files.sort(key=lambda p: (os.path.getsize(p), os.path.relpath(p, target_dir)))
    else:  # 'name'
    ...
    for i, filepath in enumerate(matched_files):
        ...
        if args.regex_find:
            try:
                ...
                sys.stderr.write(f"Error: Invalid regular expression pattern '{args.regex_find}': {e}\n")
                sys.exit(1)
        ...
        if args.number:
            ...
            try:
                ...
                sys.stderr.write(f"Error: Invalid --number-format key: {e}. Only {{name}} and {{num}} are supported.\n")
                sys.exit(1)
    ...
    try:
        ...
        sys.stderr.write(f"Warning: Renaming completed, but failed to save history file: {e}\n")
