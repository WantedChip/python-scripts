...
def test_date_cleanup():
    ...
    # YMD format input
    assert file_renamer.clean_dates_in_stem("file_2023_05_12", "YMD") == "file_2023-05-12"
    assert file_renamer.clean_dates_in_stem("file_2023.5.2", "YMD") == "file_2023-05-02"
    assert file_renamer.clean_dates_in_stem("file 2023 5 02", "YMD") == "file 2023-05-02"

    # DMY format input (Year last)
    assert file_renamer.clean_dates_in_stem("file_12-05-2023", "DMY") == "file_2023-05-12"
    assert file_renamer.clean_dates_in_stem("file_2.5.2023", "DMY") == "file_2023-05-02"

    # MDY format input (Year last)
    assert file_renamer.clean_dates_in_stem("file_12-05-2023", "MDY") == "file_2023-12-05"
    assert file_renamer.clean_dates_in_stem("file_5.2.2023", "MDY") == "file_2023-05-02"

    # Invalid dates should not be matched or changed
    assert file_renamer.clean_dates_in_stem("file_15-15-2023", "DMY") == "file_15-15-2023"
    assert file_renamer.clean_dates_in_stem("file_35-05-2023", "DMY") == "file_35-05-2023"
...
def test_numbering_and_sorting(temp_workspace):
    ...
    files.sort(key=lambda p: (os.path.getsize(p), os.path.relpath(p, str(temp_workspace))))
...
def test_collision_validation(temp_workspace):
    ...
    renames_ext = [
        (str(f1), str(f3))  # f3 exists on disk but is not being renamed
    ]
...
def test_2phase_rename_case_only(temp_workspace):
    ...
    rename_list = [
        (str(f1), str(temp_workspace / "HELLO.TXT"))
    ]
...
def test_undo_partial_failure(temp_workspace):
    ...
    # 2. Step 2 (x.txt -> a.txt) failed. Since it failed in Phase 2, the temp file was restored back to x.txt.
    ...
    # 3. The history file must still exist and contain ONLY the remaining uncompleted step: a.txt -> x.txt
...
def test_dry_run_preview(temp_workspace, capstdout):
    ...
    # The file should not be renamed (lower of a.txt is a.txt, wait! a.txt to a.txt is a no-op, so it skips renaming!)
